...
namespace Realtime.Storage.Controllers
{
    ...
    public class TableRef<T> where T : class
    {
        ...
        public Task<StorageResponse<TableMetadata>> Del()
        {
        ...
        /// <returns>This table reference.</returns>
        public Task<StorageResponse<ItemSnapshot>> Push(T item)
        {
            return Task.Run(() =>
             {
                 var up = _repository.Update(item);

                 return !up.IsFaulted
                     ? new StorageResponse<ItemSnapshot>(new ItemSnapshot(up.Result.data))
                     : new StorageResponse<ItemSnapshot>(up.Result.error);
             });
        }
        ...
        /// <returns>This table reference.</returns>
        public Task<StorageResponse<TableMetadata>> Create(TableKey key, ProvisionType provisionType, ProvisionLoad provisionLoad)
        {
        ...
        /// <returns>This table reference.</returns>
        public Task<StorageResponse<TableMetadata>> Create(TableKey key, TableThroughput throughput)
        {
            return _repository.CreateTable(new TableMetadata
            {
        ...
        /// <param name="provisionLoad">Option of provision load (how the number of operations are divided between the read and write throughput).</param>
        /// <returns>This table reference.</returns>
        public Task<StorageResponse<TableMetadata>> Update(ProvisionType provisionType, ProvisionLoad provisionLoad)
        {
        ...
        /// <param name="throughput">The custom provision to apply.</param>
        /// <returns>This table reference.</returns>
        public Task<StorageResponse<TableMetadata>> Update(TableThroughput throughput)
        {
            return _repository.UpdateTable(new TableMetadata
            {
                throughput = throughput,
        ...
        /// <returns>This table reference.</returns>
        public Task<StorageResponse<IEnumerable<ItemSnapshot>>> GetItems(ItemQueryRequest<T> request)
        {
            return Task.Run(() =>
             {
                 var task = _repository.Query(request);

                 task.Wait();

                 if (task.IsFaulted)
                     return new StorageResponse<IEnumerable<ItemSnapshot>>(task.Result.error);

                 return new StorageResponse<IEnumerable<ItemSnapshot>>(task.Result.data.items.Select(o => new ItemSnapshot(o, _storage, TableMeta)));
             });
        }
